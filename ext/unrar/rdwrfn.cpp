...
int ComprDataIO::UnpRead(byte* Addr, size_t Count) {
    ...
    while (Count > 0) {
        ...
        if (UnpVolume && UnpPackedSize == 0 && (ReadSize == 0 || Decryption && (TotalRead & CRYPT_BLOCK_MASK) != 0)) {
#ifndef NOVOLUME
...
void ComprDataIO::UnpWrite(byte* Addr, size_t Count) {
    ...
    if (Cmd->DllOpMode != RAR_SKIP) {
        if (Cmd->Callback != NULL && Cmd->Callback(UCM_PROCESSDATA, Cmd->UserData, (LPARAM)Addr, Count) == -1)
        ...
        if (Cmd->ProcessDataProc != NULL) {
            ...
            _ESP = _EBX;
#endif
                           if (RetCode == 0) ErrHandler.Exit(RARX_USERBREAK);
        }
    }
#endif // RARDLL
...
void ComprDataIO::SetEncryption(bool Encrypt, CRYPT_METHOD Method, SecPassword* Password, const byte* Salt,
