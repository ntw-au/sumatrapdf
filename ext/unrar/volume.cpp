...
bool MergeArchive(Archive& Arc, ComprDataIO* DataIO, bool ShowFileName, wchar Command) {
    ...
    if (DataIO != NULL && SplitHeader) {
        bool PackedHashPresent = Arc.Format == RARFMT50 || hd->UnpVer >= 20 && hd->FileHash.CRC32 != 0xffffffff;
        if (PackedHashPresent && !DataIO->PackedDataHash.Cmp(&hd->FileHash, hd->UseHashKey ? hd->HashKey : NULL))
...
bool DllVolChange(RAROptions* Cmd, wchar* NextName, size_t NameSize) {
    ...
    if (!DllVolChanged && Cmd->ChangeVolProc != NULL) {
...
#if defined(_MSC_VER)
#if !defined(_WIN_64) && !defined(_M_ARM64)
        __asm mov ebx, esp
#endif
        ...
        _ESP = _EBX;
#endif
                       if (RetCode == 0) DllVolAborted = true;
        else CharToWide(NextNameA, NextName, NameSize);
    }
...
#if defined(_WIN_ALL) && !defined(_MSC_VER) && !defined(__MINGW32__) && !defined(_M_ARM64)
        _EBX = _ESP;
#endif
